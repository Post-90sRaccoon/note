...
* 黄框圈起来的是原有高度，cross-axis剩余空间均分给两行，因为header定高了，所以有留白，下面没有定高，就默认拉伸。
...
div{
  ...
  /*scroll 背景相对于元素本身固定 而不是随着他的内容滚动 对元素边框有效
    fixed  字动 背景相对于视口不动
    local相对于元素内容固定 如果一个元素拥有滚动机制，背景会随着元素的内容滚动，并且背景的绘制区域和定位区域是相对于可滚动的区域而不是包含他们的边框。                 */
